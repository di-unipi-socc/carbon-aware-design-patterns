...
def genera_dot_plot(df_csv, thresholds):
    # =====================================================================
    # GRAFICO 2: Dot plot — Risparmio CO2 (%) rispetto al Plain per threshold
    # =====================================================================
    ...
    for v in ['pattern', 'pattern_ca']:
        ...
        ax.scatter(
            subset['risparmio'], subset['threshold'],
            color=dot_palette[v], marker=markers[v],
            s=30, zorder=5, label=labels[v]
        )
        for _, row in subset.iterrows():
            if row['threshold'] == "265" and row['versione'] == "pattern":
                ax.annotate(
                    f"{row['risparmio']:.1f}%",
    ...
    for th_str in th_labels:
        ...
        ax.plot(
            [row_data['risparmio'].min(), row_data['risparmio'].max()],
            [th_str, th_str],
    ...
    ax.set_xlim(left=df_dot['risparmio'].min() - 12.3,
                right=df_dot['risparmio'].max() + 70)
    plt.tight_layout()
    plt.savefig("2_risparmio_co2_dot_plot.pdf", bbox_inches="tight")
    plt.close()
...
def grafico5_dual_risparmio_eta(df_th, th):
    ...
    ax1.plot(ttls_str, pivot_emissions['risparmio_pattern'],
            color=palette["pattern"], marker='o', markersize=8, linewidth=3,
            linestyle='-', label='Risparmio % (Pattern)', zorder=10)
    
    ax1.plot(ttls_str, pivot_emissions['risparmio_pattern_ca'],
            color=palette["pattern_ca"], marker='s', markersize=8, linewidth=3,
            linestyle='-', label='Risparmio % (Pattern CA)', zorder=10)
    ...
    # =========================================================
    # 3. PLOT ASSE DESTRO: STRIPPLOT DELL'ETA
    # =========================================================
    ax2 = ax1.twinx()
    ...
    sns.stripplot(data=df_cache, x='ttl_str', y='eta', hue='versione',
                palette=palette, dodge=True, alpha=0.20, jitter=True, size=4, ax=ax2, zorder=5)
    ...
    ax1.legend(lines1 + handles2, labels1 + labels2,
               loc='upper left', framealpha=0.9, fontsize=7, ncol=2)
    ...
    plt.tight_layout()
    out_name = f"5_dual_risparmio_eta_th_{th}.pdf"
    plt.savefig(out_name, bbox_inches="tight")
    plt.close()
    print(f" Generato: {out_name}")
...
def main():
    ...
    df_csv = pd.read_csv("risultati_test.csv", sep=";")
    ...
    for th in thresholds:
        ...
        plt.tight_layout()
        plt.savefig(f"1_co2_trend_th_{th}.pdf", bbox_inches="tight")
        plt.close()
        ...
        sns.lineplot(data=mean_emissions_ttl, x='ttl', y='co2_emessa', hue='versione',
                    marker='o', linewidth=2, palette=custom_palette)
        ...
        plt.tight_layout()
        plt.savefig(f"3_emissioni_vs_ttl_th_{th}.pdf", bbox_inches="tight")
        plt.close()

        # =====================================================================
        # GRAFICO 4: Boxplot Eta del Dato (ETA) in base al TTL (solo cache)
        # =====================================================================
        plt.figure(figsize=(8, 5))
        df_cache = df_th[df_th['versione'].isin(['pattern', 'pattern_ca'])]
        
        sns.stripplot(data=df_cache, x='ttl', y='eta', hue='versione',
            palette={"pattern": "#e16f0b", "pattern_ca": "#1a6498"},
            dodge=True, alpha=0.5, jitter=True, size=4)
        ...
        plt.legend(title="Versione", fontsize=8, title_fontsize=8)
        plt.tight_layout()
        plt.savefig(f"4_stripplot_eta_th_{th}.pdf", bbox_inches="tight")
        plt.close()

        grafico5_dual_risparmio_eta(df_th, th)

    genera_dot_plot(df_csv, thresholds)
    print("\n Tutte le 12 immagini sono state generate con successo!")
